...
| **联网搜索** | 网络搜索（Tavily、Bing 或 DuckDuckGo），并把结果网页读取为纯文本 |
...
- **个人资料**：保存填表常用的信息——姓名、出生日期、国籍、身份证和护照号码、联系方式，以及你自定义的字段。数据只存在本地，助手只有在你同意后才会写入。
...
| **导航** | 缩略图、PDF 书签、为没有书签的 PDF 用 AI 生成目录、跳转页码、可点击的链接 |
| **缩放** | 适合宽度、适合页面，用触控板或 Ctrl + 滚轮平滑缩放 |
| **搜索** | 全文搜索，高亮所有匹配结果 |
| **选中文字菜单** | 高亮、下划线、删除线、复制、解释、翻译，或把选中内容发到对话 |
| **批注** | 画笔、荧光笔、下划线、删除线、矩形、椭圆、直线、箭头和文本框，颜色和粗细可调 |
| **表单与签名** | 可以直接输入的真实表单字段；手写并保存签名 |
| **对话上下文** | 用 `@3` 或 `@2-4` 附加页面，也可以附加选中的文字或框选的区域 |
| **对话** | 多个对话记录、可点击的页码引用、重试、复制、分享和语音输入 |
| **导出** | 带批注的 PDF、纯图片的涂黑 PDF、Markdown 笔记、TSV 抽认卡、CSV 表格、BibTeX 和 `.ics` 日历 |
| **设置** | 中文或英文、浅色或深色、模型和思考强度、可分别开关文档编辑和联网搜索 |
...
| Base URL | `https://api.deepseek.com` | 任何 OpenAI 兼容接口都可以 |
| 模型 | `deepseek-flash` | Agent 功能需要支持工具调用的模型 |
| 页面格式 | 自动 | 模型不能识图时选择 **仅文字** |
| 思考强度 | 高 | 关、低、高或最高 |
| 文档编辑 | 开 | 关闭后助手不会修改 PDF |
| 联网搜索 | 开 | 填入 Tavily Key 效果更好；否则依次使用 Bing 和 DuckDuckGo |
...
- **联网搜索可见可控。** 搜索词会发送给 Tavily、Bing 或 DuckDuckGo，结果页面可能会被读取。联网搜索可以关闭。
...
| `src/profile.js`、`src/todos.js` | 填表用的个人资料字段；笔记本待办页上的事项 |
