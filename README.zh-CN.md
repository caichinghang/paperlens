...
这个助手不只是回答关于 PDF 的问题。它可以查看页面、搜索文档和网络、填写表单、高亮和批注、原地翻译整页，并把笔记存到这份文档专属的笔记本里。你可以选择一个**插件**——学习备考、论文研究、合同审阅、表单办事或报告分析——它会优先使用适合的技能。
...
![“表单办事”插件中的“填写表单”：根据个人资料填好字段、选好学院和时段、勾选同意，签名已确认并填上日期](docs/screenshots/zh/forms.png)

### 4. 插件

插件告诉助手你在做哪一类工作。在 `/` 菜单顶部选择一个，它会在整个对话中保持生效。

每个插件都会在系统提示词中加入自己的指引，并设定一组**优先技能**。助手会自己从中挑选合适的技能，不需要你点名；这些技能也会作为快捷按钮显示在输入框上方。其他技能和工具仍然可以使用。
...
![/ 菜单：五个插件排在技能列表上方](docs/screenshots/zh/scenarios.png)
...
- **自动推理只发送问题。** 填入 TypeSafe 密钥并打开 **自动** 后，只有你的问题会发送给 TypeSafe，供 Jev 挑选推理强度；文档、页面文字和回复都不会发送。发送前会先遮盖隐私资料，打招呼一类的问题根本不会发出。未填密钥时没有这个开关，也不会发送任何内容。
...
| **数学公式** | [KaTeX](https://katex.org/)（内置于 `vendor/katex`），渲染聊天回复和笔记本笔记中的 LaTeX 公式 |
| **代码高亮** | [highlight.js](https://highlightjs.org/)（内置于 `vendor/hljs`），为聊天回复里的代码块按语言上色 |
...
| `src/typesafe.js` | 请 TypeSafe 的 Jev 模型判断一个问题该用多大的推理强度 |
...
PaperLens 以 [MIT 许可证](LICENSE) 发布。PDF.js 采用 Apache 2.0 许可证。KaTeX 采用 MIT 许可证。highlight.js 采用 BSD-3-Clause 许可证。
